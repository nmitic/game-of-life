...
const Game: React.FC <UniverseProps> = ({size, organismSize = '20px'}) => {
  ...
  const [universe, setUniverse] = useState<UniverseType>(makeUniverse(size, false))
  ...
  const runGameBackward = (history: UniverseType[]): void => {
    ...
    const gameId = setTimeout(() => runGameBackward(pastHistory), 1)
    setGameId(gameId)
  }
  
  const runGame = (currentUniverse: UniverseType, history: UniverseType[]): void => {
    setPlaying(true)
    const [newUniverse, newHistory] = runOneStepForward(currentUniverse, history)
    ...
    const gameId = setTimeout(() => runGame(newUniverse, newHistory), 1)
    setGameId(gameId)
  }
  ...
  const runOneStepForward = (currentUniverse: UniverseType, history: UniverseType[]) : [UniverseType, UniverseType[]] => {
  ...
  return (
    <GameStyled>
      ...
      />
      <DeclaimerStyled>You can click on organism to make it alive or dead and set initial universe state.</DeclaimerStyled>
      <GameGrid universe={universe} setUniverse={setUniverse} size={size} organismSize={organismSize}/>
      <GlobalStyle />
    </GameStyled>
  );
}
